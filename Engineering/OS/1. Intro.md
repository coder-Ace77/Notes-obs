...
![[Pasted image 20251214151301.png]]
...
- decode what this instruction means
- execute it
...
Then control unit decodes the operation meaning what to do on which registers and if there is any memory access. If that is that data needs to be loaded from memory address into some general purpose register.
...
Observe that all the three steps fetch , decode and ececute need different component of cpu and thus can be done parallelly. Infact this is called Pipelining and is the reason why cpu's are so fast. 
...
For computer to start at power up it needs intial program to run. The first program to run is called bootstrap program (firmware) and is stored in ROM. These programs are written by motherboard vendor and some of its emaples are BIOS and UEFI. 
