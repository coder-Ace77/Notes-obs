...
![[Pasted image 20251216140852 1.png]]
...
![[Pasted image 20251216141315 1.png]]
...
![[Pasted image 20251216141722 1.png]]
...
![[Pasted image 20251216142805 1.png]]
...
Generally people think that context switching is needed only when one process is swapped by another process. But it is also needed when system call is done. As we need to load the kernel program. So entire TLB cache is flushed all the `L1` and `L2` caches are removed etc. This is sometimes very high cost when the system depends on `L1` and `L2` cahces for performace for example the case of `Redis`. Addtionally the State save and switching of stacks is needed.
