...
The real difficulty is knowing the time of next cpu burst. Although the SJF algorithm is optimal, it cannot be implemented at the level of short-term CPU scheduling. With short-term scheduling, there is no way to know the length of the next CPU burst.

One way is to predict the time of next burst. We expect that the next CPU burst will be similar in length to the previous ones. By computing an approximation of the length of the next CPU burst, we can pick the process with the shortest predicted CPU burst.
...
![[Pasted image 20260111094433.png]]
...
![[Pasted image 20260111094518.png]]
