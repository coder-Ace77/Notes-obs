...
All the atomic things lie in `java.util.concurrent.atomic`. There are three main functions one should know - 

`getAndIncrement()`, `addAndGet(int)`, `compareAndSet(expected, update)`
...
Optimistic locking is a strategy where you assume that multiple transactions can complete without affecting each other. Instead of locking a record the moment you read it (Pessimistic Locking), you let the transaction proceed and only check for conflicts at the very end when you try to save the data.

#### Using atomic variables

Atomic varitables as we know of allow some operations using special harware assistance to do operations such as `compareAndSet` now with this we can maintain version number and do optimistic locking. 

The `compareAndSet(expectedValue, newValue)` method is an atomic instruction supported by modern CPU hardware (like the `CMPXCHG` instruction on x86). It does the following in a single, indivisible step:

1. It checks if the variable's current value is equal to `expectedValue`.
2. **If yes:** It updates the variable to `newValue` and returns `true` (success).
3. **If no:** It does nothing to the variable and returns `false` (failure), indicating another thread modified the value first.
...
public void updateValue() {
    ...
    while (!success) {
        ...
        // 2. Calculate new state based on current
        int next = current + 1;
        // 3. Attempt atomic update (Optimistic check)
        success = atomicInt.compareAndSet(current, next);       
        // If false, the loop runs again (Retry)
    }
}
```

This is most simplest idea. So you read first check for change in version or value and then later on again check and update instantly. 

The most common use case is implementing counters that don't block threads. Standard locks are essentially "heavy" for simple math operations.

CAS is the engine behind Java's non-blocking collections, such as `ConcurrentLinkedQueue` or `ConcurrentSkipListMap`.

CAS is ideal for managing state flags where valid transitions depend strictly on the current state.

Example - Stating service iff its stopped. 

```java
AtomicReference<State> state = new AtomicReference<>(State.STOPPED);

public boolean startService() {
    // Optimistically attempt to transition from STOPPED to RUNNING
    if (state.compareAndSet(State.STOPPED, State.RUNNING)) {
        // We successfully acquired the "lock" on the state
        runStartupLogic();
        return true;
    } else {
...
Beyond the basic `Atomic` classes, Java provides several sophisticated tools that leverage optimistic locking strategies for specific performance scenarios.

Java provides optimisatic locking since java 8 using `StampedLock`. 

### Stampedlock

This is the most powerful "true" optimistic lock in Java. It is designed to replace `ReentrantReadWriteLock` in high-performance scenarios where reads vastly outnumber writes.

Unlike standard locks, `StampedLock` has a special mode called **Optimistic Read**. It doesn't actually block writers while you are reading. Instead, it gives you a "stamp" (a validation token). You read your data, and then check if the stamp is still valid.

**How it works:**

1. Get a stamp (`tryOptimisticRead()`).
2. Copy variables to local memory.
3. Check if the stamp is still valid (`validate(stamp)`).
4. If valid: You are done (super fast!).
5. If invalid: A writer changed data while you were reading. You must retry or upgrade to a pessimistic lock.
...
class Point {
    ...
    double distanceFromOrigin() {
        ...
        // 2. Read state into local variables
        double currentX = x;
        double currentY = y;
        // 3. Check if a write occurred since step 1
        if (!sl.validate(stamp)) {
            // A write happened! Optimistic read failed.
            // Fallback: Acquire a real pessimistic read lock
            stamp = sl.readLock();
            try {
                currentX = x;
                currentY = y;
        ...
        // 4. Do calculation on local variables (safe now)
        return Math.sqrt(currentX * currentX + currentY * currentY);
    }
}
```

_Note: `StampedLock` is **not reentrant**, so be careful not to call a locked method from within another locked method._

#### Atomic longAddr

`AtomicLong` uses a simple CAS loop. Under very high contention (e.g., thousands of threads updating a counter), the CAS operations fail repeatedly, causing CPU spikes due to retries.

`LongAdder` optimizes this by **sharding the value**. Instead of one single variable, it maintains an array of variables ("cells"). Different threads update different cells to avoid conflict. When you ask for the `sum()`, it adds them all up.

 - **Use Case:** Metrics, request counters, statistics gathering.
 - **Optimistic Strategy:** It tries to CAS the base value. If that fails due to contention, it doesn't spin; it picks a cell in its internal array and updates that instead (reducing contention probability).
...
// Thread 1
counter.increment(); 
// Thread 2 (No conflict with Thread 1 effectively)
counter.add(5);
// Reader
long total = counter.sum(); // Sums up all internal cells
```