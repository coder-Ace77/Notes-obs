...
We can check for null vaues by `is` clause. 
...
A **Trigger** is essentially a "stored procedure" that doesn't wait for you to call it. Instead, it **automatically fires** (executes) in response to a specific event on a particular table or view.
...
- **The Event:** What action sets it off? (INSERT, UPDATE, or DELETE).
- **The Timing:** Does it happen _BEFORE_ the data is saved, or _AFTER_?
- **The Action:** The actual SQL code that runs (the "payload").

It is useful to create the logs and automatically insertion of something in table. 

There are row level triggers which run **once for every row** affected by the SQL statement. And statement level triggers. These execute **once per SQL statement**, regardless of how many rows are affected. Even if you update 1,000 rows, the trigger runs only once. 

If you have complex triggers on a table where you are inserting thousands of rows per second, it will significantly slow down your database.

#### Delete vs truncate 

In database management, `DELETE` and `TRUNCATE` might seem to do the same thing—remove data—but they operate very differently under the hood. Choosing the wrong one can lead to performance issues or accidental data loss.

`DELETE` is a **Data Manipulation Language (DML)** command. It is used to remove specific rows from a table using a `WHERE` clause. If you don't provide a `WHERE` clause, it deletes all rows, but it does so one by one. 

- **Row-level control:** You can delete specific entries (e.g., `DELETE FROM users WHERE id = 5`).
- **Logging:** It records every single row deletion in the transaction log. This makes it slower for large datasets but allows you to **ROLLBACK** (undo) the operation if you are inside a transaction.
- **Triggers:** If the table has "on delete" triggers, they will fire for every row removed.

`TRUNCATE` is a **Data Definition Language (DDL)** command. Think of it as a "reset" button for a table. It removes all rows by deallocating the data pages used to store the table data.

- **All-or-nothing:** You cannot use a `WHERE` clause. It wipes the entire table.
- **Speed:** It is much faster than `DELETE` because it doesn't scan every row or log individual row deletions. It simply logs the page deallocations.
- **Identity Reset:** If your table has an auto-incrementing ID (Identity column), `TRUNCATE` resets the counter back to its seed value. `DELETE` does not.
- **Triggers:** It does not fire any delete triggers because it doesn't operate on individual rows.