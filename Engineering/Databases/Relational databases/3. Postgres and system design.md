...
In postgres everything needs to connect to it first be it application server or simple client user. That means each application which is wanting to get connected to postgres needs a piece of client code as well which is often called as postgres driver. 
