...
Keys are strings and values are strings, numbers, boolean, lists, or other mappings. Values can also be other full fledged objects. Note after a key `:` there should be a space. 
...
Note that lists can be a top level element without any key.
...
etc. 
#### 3.Nested structures: Dictionary/Map
...
Note that hobbies is an array having two elements - `reading,hiking`. Person is a dictionary with three fields `name` , `age` and `hobbies`. A dictionry can not have direct value. The rule is that a key can either have a direct value or an array or an hashmap. 

You can have dictionaries inside an array. 
