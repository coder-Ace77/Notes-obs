...
#### Otimistic solution to bounded buffer problem 
...
class OptimisticBuf {
    // 1. Use AtomicIntegerArray for thread-safe array access
    private final AtomicIntegerArray buffer;
    
    // 2. Use Atomic pointers instead of int
    private final AtomicInteger in = new AtomicInteger(0);
    private final AtomicInteger out = new AtomicInteger(0);
    private final int capacity;

    // We need a "special value" to represent an empty slot 
    // to handle the race condition where a pointer moves before data is written.
    private static final int EMPTY = -1; 

    public OptimisticBuf(int n) {
        this.capacity = n;
        this.buffer = new AtomicIntegerArray(n);
    ...
    public void put(int x) {
        ...
        while (true) {
            currentIn = in.get();
            int currentOut = out.get();

            // 1. Check if Full (Mathematical check instead of Semaphore)
            if (((currentIn + 1) % capacity) == currentOut) {
                Thread.yield(); // Buffer is full, yield CPU to let Consumer run
                continue;       // Retry loop
            }

            // 2. Calculate next index
            nextIn = (currentIn + 1) % capacity;

            // 3. CAS: Try to reserve this slot
            // If currentIn changed (another producer got here first), returns false -> loop repeats
            if (in.compareAndSet(currentIn, nextIn)) {
                
                // CRITICAL SECTION (We own 'currentIn' now)
                
                // Wait until the Consumer has fully cleared this slot (set it to EMPTY).
                // This handles the "lap around" case in ring buffers.
                while (buffer.get(currentIn) != EMPTY) {
                    Thread.yield();
                }

                // Write the data
                buffer.set(currentIn, x);
                return; // Success!
            }
        }
    }
    ...
    public int get() {
        ...
        while (true) {
            currentOut = out.get();
            int currentIn = in.get();

            // 1. Check if Empty
            if (currentOut == currentIn) {
                Thread.yield(); // Buffer is empty
                continue;
            }
            // 2. Calculate next index
            nextOut = (currentOut + 1) % capacity;

            // 3. CAS: Try to claim the item
            if (out.compareAndSet(currentOut, nextOut)) {
                
                // CRITICAL SECTION (We own 'currentOut' now)
                
                // DATA RACE FIX:
                // The Producer might have moved 'in' pointer, but hasn't finished 
                // writing the data to the array yet. We must wait for data.
                int data = buffer.get(currentOut);
                while (data == EMPTY) {
                     Thread.yield();
                     data = buffer.get(currentOut);
                }

                // Clear the slot for the next usage (reset to EMPTY)
                buffer.set(currentOut, EMPTY);
