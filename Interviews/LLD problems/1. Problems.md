...
# LLD Practice Sheet — 50 Problems

## How to use this

Timebox each problem to **45 minutes**, and produce the same five artifacts every single time:

1. **Requirements** — 6–8 functional bullets + 2–3 non-functional (concurrency, scale, latency). Explicitly write down what you're leaving out.
2. **Core entities** — nouns → classes. Mark which are entities (have identity), which are value objects, which are services.
3. **Class diagram / interface skeleton** — method signatures with types. No bodies except where the logic _is_ the answer.
4. **One flow written out end to end** — the happy path, call by call.
5. **Concurrency + extensibility** — what breaks with 100 concurrent users, and where the next feature plugs in.

Rules that make this actually useful:

- **Write compilable code**, not pseudocode. Pick one language and stay there (Java/Go/Python — whichever you'll be interviewing in).
- **No premature patterns.** Name a pattern only after you've felt the pain it solves. Interviewers can tell.
- **Redo, don't just move on.** Problems 1–20 should each be attempted twice, a week apart. The second pass is where the learning is.
- Track state below: `☐` not started · `◐` attempted · `✔` clean second pass.

Suggested pacing: 2 problems/day for Tier 1 (~2 weeks), 1/day for Tier 2 (~3 weeks), 1 every other day for Tier 3.

---

## Tier 1 — Foundations (1–20)

Small scope, big on OOP fundamentals. The point isn't the problem, it's the concept in column 3.
...
- Tension: price-time priority matching, partial fills, and cancel/modify racing with a match. The data structure choice (heaps vs sorted map of price levels with FIFO queues) _is_ the answer.
...
Before you call any problem done, ask:

- Could I add the next obvious feature **without editing existing classes**? (Open/Closed)
- Is every class's dependency an **interface I could fake in a test**?
- Where's the **shared mutable state**, and what protects it?
- What happens on **partial failure** halfway through the flow?
- Which of my abstractions exists only because a pattern felt nice? Delete it.