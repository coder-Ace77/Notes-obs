...
Given n computers and m batteries. We can plug one battery in one computer and computer will be running. Each battery has some health meaning it can keep computer running for that time. Now your task is to find max time for which we can keep all n computers running. It takes `0` time to change battries.

### Solution

The problem is tricky one because by binary search we can guess the say some time `t`. But how do you verify it. The idea is to understand that if we want our computers to run for time `t`. Then any battery with heath `t1`>`t` is actually of no use to us. after time `t` and reasoning is pretty simple we will never be able to use a bettery for more time `t`. Now how this helps. Actually we can now sum the normalized times `t[i] = min(t[i],t)`. Now every time added can actually be used. And so now sum needs to just be >=`n*t`. This solves problem. 

```cpp
#define ll long long
class Solution {
public:
    ll maxRunTime(int n, vector<int>& arr){
