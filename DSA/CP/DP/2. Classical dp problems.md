...
For this problem its a little tricky to choose the state. We choose state as follows - 

```
dp[i][j] = max score such that range between i and j is collapsed excl i and j
```

 Now suppose we want to find answer for range `i,j` how to do that well we can say if k was the last element in the range `i,j` to be burst then on bursting k we get `a[i]*a[k]*a[j]` and other result was `dp[i][k]+dp[k][j]` this is our recurrence. 
...
We have padded `1` on both sides to make our base cases simpler. 

### Palindromic partitioning

Given a string `s`, partition `s` such that every substring of the partition is a palindrome.Return _the **minimum** cuts needed for a palindrome partitioning of_ `s`.

#### Solution 

Observe that we can choose state as  - 

```
dp[i] = min cuts in prefix [0:i]
```

Now we can say lets cut at point j<=i and we can do cut if `s[j:i]` is palindrome. 

Now testing if its palindrome can be precomputed using dynamic programming. 

```
isPal[i][j] = true if s[i]==s[j] && isPal[i+1][j-1] is true
```
...
public:
    int minCut(string s) {
        ...
        // Fill the table (bottom-up because i depends on i+1)
        for (int i = n - 1; i >= 0; i--) {
            for (int j = i; j < n; j++) {
                // If outer characters match AND (inner part is pal or substring length < 2)
        ...
        // cuts[i] = min cuts for prefix s[0...i]
        vector<int> cuts(n);
        for (int i = 0; i < n; i++) {
            int minVal = i; // Maximum possible cuts for length i+1 is i

            for (int j = 0; j <= i; j++) {
                if (isPal[j][i]) {
                    if (j == 0) {
                        // Entire prefix is a palindrome, no cuts needed
                        minVal = 0;
                    } else {
                        // Cut between j-1 and j
                        minVal = min(minVal, cuts[j - 1] + 1);
                    }
                }
            }
            cuts[i] = minVal;
