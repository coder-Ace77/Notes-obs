...
DP problems can be classified into `9` major paradigms - 

### Linear and sequence dp

This is the most common paradigm. You process an array/string from left to right, and the state usually depends on a prefix.

##### Prefix dp
In prefix dp the `ith` state of `dp` depends only on the `i-1th` state. Note that index `i` is majorily defined over some sequence or an array. Now the optimal solutions to such kind of problems is `O(n)` in time and `O(1)` in space complexity. 

Example - House robber , max subarray sum

#### Coordinate dp
Here the answer strictly does not depends upon the previous state. Rather on some earlier index. Most of the times the `dp` solution of such type will be `O(n^2)` as we will be required to go back. Notable examples include `LIS`

#### Two sequence dp
Here we maintain two sequences and the index of first seq can depend on any index of second seq. To solve such problems we have to maintain the `2D` table. e.g., _Longest Common Subsequence (LCS), Edit Distance_)


### Counting and combinatorial dp 

Instead of finding an "optimal" value (min/max), you count total ways.
#### Complementary counting 
Subtracting "overcounted" cases. Note that when we try to extend the seq based on some counting this substraction rule required. Example `Distinct subsequnece 2`
#### Partition dp 
Counting ways to split a number or string into components. (e.g., _Integer Partition, Ways to Decode_).
#### Inclusion exclusion
Using the principle of inclusion-exclusion within a DP state. This is most hardest form of combinatorial dp. 

### Knapsack and resource constraint dp

In these problems mostly a constraint over resource is given and we are required to solve it by adding the constraint to the `dp state`. 

#### 0/1 knapsack
Items cannot be reused.
#### Unbounded knapsack
Items can be used infinitely. (e.g., _Coin Change II_).
#### Subset sum
Finding if given sum is possible.

### Interval dp

The state is defined by a range `[i,j]`. You solve for small intervals first and merge them to solve larger ones.
Example - _Matrix Chain Multiplication, Burst Balloons, Optimal Binary Search Tree_.

### Bitmask dp

Used when the "state" involves a set of objects. If you have N objects, a bitmask (an integer where the k-th bit is 1 or 0) represents the subset of objects already used

**Constraint Hint:** Usually N≤20.
**Common Problems:** _Traveling Salesperson Problem (TSP), Assignment Problem, Smallest Sufficient Team_.

### DP on trees

The state is calculated for each node based on its children.

**Sub-Paradigms:**
- **Rerooting DP:** Solving the problem for every node as a root in O(N) (e.g., _Sum of Distances in Tree_).
- **Diameter/Path DP:** Finding the longest path or specific property through a node.

### Digit dp

Counting numbers in a range `[L,R]` that satisfy a specific property (e.g., "how many numbers between 1 and 1018 contain the digit 7?").

### Probability and expectation dp

The transition involves multiplying by probabilities rather than just adding/subtracting.**Common Problems:** _Knight Probability in Chessboard, Dice Rolls_.

```
E[x] = sum(P[i]*V[i])
```

### State compression and dp optimisation

Advanced techniques to reduce the time complexity of standard DP.
**Sub-Paradigms:**
- **Convex Hull Trick (CHT):** Optimizing dp[i]=min(mj​⋅xi​+cj​).
- **Divide and Conquer Optimization:** Reducing O(N2) to O(NlogN).
- **Matrix Exponentiation:** Solving linear DP (like Fibonacci) in O(logN) by treating transitions as matrix multiplications.
