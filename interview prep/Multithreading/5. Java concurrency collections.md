...
**Locking contention** occurs when multiple processes or threads try to access the same resource (like a row in a database, a file, or a shared variable in memory) at the same time, but the resource is "locked" by one of them. On high performance needed systems it adds very high cost. 

**Reduced Scalability:** Adding more CPU cores doesn't make the application faster because threads are just waiting longer for the same locks.

**High Context Switching:** The OS spends more time swapping threads in and out of the CPU than actually executing code.

**Increased Latency:** Specific tasks take much longer to complete because they are stuck in a queue.

Ways to Reduce Contention - 

1. Reduce lock granuality -  Instead of locking an entire database table, lock only the specific row being modified.
2. For high-performance scenarios, use "Atomic" operations (like `AtomicInteger` in Java or `std::atomic` in C++). These use hardware-level instructions to update values without needing a traditional lock.
3. Another way to reduce contention is to do optimisatic locking which means don't lock and in the mean time if resource had changed we undo and retry. 
